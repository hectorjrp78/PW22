...
function jugar(turno = 0) {
  let sw = false; 
  if (turno === 0) {
    // Si es el primer turno, asignar el jugador1 como el actual
    jugadorActual = jugador1;
  }
  let posicion = parseInt(
    prompt("Jugador " + jugadorActual + ", ingrese una posición (1-9): ")
  );
  if (!validar_posicion(posicion)) {
    // Verificar si la posición es válida
    console.log("Posición inválida. Intente de nuevo.");
    sw = true;
  } else if (validar_posicion_ocupada(posicion)) {
